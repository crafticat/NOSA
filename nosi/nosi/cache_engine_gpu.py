...
class CacheEngine:
    ...
    def prefill_update(self, key_states, value_states, kv_bias, current_batch_pos, total_bsz):
        ...
        if current_batch_pos == 0:
            ...
            self._k_cpu = torch.empty((total_bsz, S+self.max_gen_len, H, D), dtype=self.dtype, device=self.device)
            self._v_cpu = torch.empty((total_bsz, S+self.max_gen_len, H, D), dtype=self.dtype, device=self.device)
...
class InfLLMv2Cache(DynamicCache):
    def __init__(self,
                 config,num_hidden_layers: Optional[int] = None, has_kv_bias: bool = False) -> None:
